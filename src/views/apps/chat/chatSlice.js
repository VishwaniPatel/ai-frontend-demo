...
export const getItems = () => async (dispatch, getState) => {
  ...
  if (state.chat.selectedChat === null)
    dispatch(
      chatSetSelectedChat(items.filter((x) => x.messages.length > 0)[0])
    );
};
...
export const addTextToChat =
  ...
  async (dispatch) => {
    if (text !== "" && text.length > 0) {
      dispatch(setLoading(true));
      const response = await axios.put(`${SERVICE_URL}/apps/chat/addText`, {
        id: chat.id,
        text,
      });
      const items = response.data;
  ...
  async (dispatch) => {
    if (attachments !== null && attachments.length > 0) {
      ...
      const response = await axios.put(
        `${SERVICE_URL}/apps/chat/addAttachments`,
        { id: chat.id, attachments }
      );
      const items = response.data;
      dispatch(receiveService({ items, loading: false }));
      dispatch(chatSetSelectedChat(items.filter((x) => x.id === chat.id)[0]));
...
export const selectChat =
  ...
  async (dispatch) => {
    if (chat !== null) {
      dispatch(setLoading(true));
      const response = await axios.put(`${SERVICE_URL}/apps/chat/read`, {
        id: chat.id,
      });
      const items = response.data;
      dispatch(receiveService({ items, loading: false }));
